...
// youtube-nocookie embeds, lazy-loaded. First 6 show; the rest sit behind
// "View more".
const TESTIMONIALS = [
    ...
    >
      <Container>
        <div className="max-w-[820px] mb-16 md:mb-20">
          ...
          <p className="mt-6 text-[1.2rem] text-[#4A4640] leading-[1.65] max-w-[680px] font-normal">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-7">
          {visible.map((id, i) => (
            <Reveal key={id} delay={(i % 3) * 80}>
              <div className="relative aspect-video overflow-hidden rounded-2xl border border-[#DDD3BC] bg-[#EBE5D3] shadow-[0_1px_2px_rgba(15,14,12,0.05),0_20px_40px_-28px_rgba(15,14,12,0.22)]">
                <iframe
        ...
        {TESTIMONIALS.length > INITIAL_COUNT && (
          <div className="mt-12 flex justify-center">
            <button
              ...
              className="inline-flex items-center gap-2 px-6 py-3 text-[15px] font-medium text-[#0F0E0C] bg-transparent border border-[#0F0E0C]/25 rounded-[10px] hover:bg-[#EBE5D3] hover:border-[#0F0E0C]/50 transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#2F4B3A]"
            >
              {showAll ? (
                <>
                  View less
